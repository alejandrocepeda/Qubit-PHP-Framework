...
class Qubit_Rand{
    ...
    public static function getAlternativeGenerator()
    {
        ...
        if (!class_exists('RandomLib\\Factory')) {
            throw new Qubit_Exception(
                'El RandomLib pseudoaleatorio generador de números (PRNG)
                debe estar instalado en la ausencia de la OpenSSL y
                extensiones Mcrypt'
            );
        }
    ...
    public static function getInteger($min, $max, $strong = false)
    {
        if ($min > $max) {
            throw new Qubit_Exception(
                'El parámetro min debe ser menor que el parámetro max'
            );
        }
        $range = $max - $min;
        if ($range == 0) {
            ...
            throw new Qubit_Exception(
                'La rango ofrecido es demasiado grande para generar'
            );
        }
    ...
    {
        if ($length < 1) {
            throw new Qubit_Exception('Longitud debe ser >= 1');
        }
